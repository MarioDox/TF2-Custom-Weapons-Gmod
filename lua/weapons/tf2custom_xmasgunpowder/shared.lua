...
if (CLIENT) then
	...
	SWEP.PrintName			= "Xmas Gunpowder"	
	SWEP.Slot				= 2
	SWEP.SlotPos			= 1
	SWEP.DrawSecondaryAmmo = false
	SWEP.DrawAmmo		= false
	SWEP.Type =  "Christmas ball"
	SWEP.Stats =  "Throw it to the ground to freeze the enemies in range for a short period"
	SWEP.Stats2 = ""
	SWEP.Stats3 = ""
	SWEP.Stats4 = ""
	SWEP.Stats5 = ""
	...
	SWEP.Stats11 = "Has a cooldown of 15 seconds"
	SWEP.Stats12 = "-100% Damage penality"
	SWEP.Stats13 = ""
	SWEP.Stats14 = ""
	SWEP.Stats15 = ""
	SWEP.Stats16 = ""
	SWEP.Description = "Why pirates have an hook to the hand and not a stick?"
	SWEP.Set = "Christmas War"
end

SWEP.VElements = {
	["gun"] = { type = "Model", model = "models/player/items/soldier/xms_soldier_ornaments.mdl", bone = "weapon_bone", rel = "", pos = Vector(26.856, 54.229, -8.171), angle = Angle(1.248, -66.32, 35.198), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["gun"] = { type = "Model", model = "models/player/items/soldier/xms_soldier_ornaments.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(13.618, 4.973, 56.847), angle = Angle(90, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Throw()
    if SERVER then
        ...
        local grenade = ents.Create("tf2custom_proj_xmasgunpowder")
	...
	self:Remove()
	timer.Create("xmasgrenaderespawn"..oldowner:EntIndex(),15,1,function()
	if SERVER then
	oldowner:Give("tf2custom_xmasgunpowder")
	end
	end)
end

hook.Add("PostPlayerDeath","TF2Customgrenadexmasgiveply",function(ply)
if timer.Exists("xmasgrenaderespawn"..ply:EntIndex()) then timer.Remove("xmasgrenaderespawn"..ply:EntIndex()) end
end)
	...
	function SWEP:ViewModelDrawn()
		...
		for k, name in ipairs( self.vRenderOrder ) do
			...
			if (v.type == "Model" and IsValid(model)) then
				...
				model:DrawModel()
				model:ManipulateBoneScale( model:LookupBone("prp_grenade_top"), Vector(0,0,0) )
				render.SetBlend(1)
