...
if (CLIENT) then
	...
	SWEP.PrintName			= "Wind-stick"	
	SWEP.Slot				= 2
	SWEP.SlotPos			= 1
	SWEP.DrawSecondaryAmmo = false
	SWEP.DrawAmmo		= false
	SWEP.Type =  "Boomerang?"
	SWEP.Stats =  "Alt-Fire: Throw"
	SWEP.Stats2 =  "+10% attack speed rate"
	SWEP.Stats3 = ""
	SWEP.Stats4 = ""
	SWEP.Stats5 = ""
	SWEP.Stats6 = ""
	...
	SWEP.Stats15 = ""
	SWEP.Stats16 = ""
	SWEP.Description = "Even if it has the shape of a windmill, it doesn't produces wind, but can be thrown."
end

SWEP.VElements = {
	["gun"] = { type = "Model", model = "models/player/items/engineer/bet_wingstick.mdl", bone = "weapon_bone", rel = "", pos = Vector(-32.78, 20.16, 9.765), angle = Angle(162.908, -39.314, 14.211), size = Vector(1.177, 1.177, 1.177), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["gun"] = { type = "Model", model = "models/player/items/engineer/bet_wingstick.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-6.955, 17.283, 32.257), angle = Angle(0, -91.541, -66.89), size = Vector(1.177, 1.177, 1.177), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
if self.Owner:GetEyeTrace().HitPos:Distance(self.Owner:GetShootPos()) < 80 then self:EmitSound("weapons/medigun_no_target.wav") return end
self:Holster2()
...
timer.Create("windstick1"..self.Owner:EntIndex(),.4,1,function()
if SERVER then
local ent = ents.Create("tf2custom_proj_windstick")
if self.Owner:GetEyeTrace().HitPos:Distance(self.Owner:GetShootPos()) < 80 then
ent:SetPos(self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 10)
else
ent:SetPos(self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 80)
end
local angl = self.Owner:EyeAngles()
angl:RotateAroundAxis(self.Owner:EyeAngles():Up(),90)
ent:SetAngles(angl)
ent:Spawn()
...
hook.Add("PostPlayerDeath","Windstickno",function(ply)
if timer.Exists("windstick1"..ply:EntIndex()) then timer.Remove("windstick1"..ply:EntIndex()) end
end)
