local RevolverLimit = { }
RevolverLimit[ "357" ] = 150

if (CLIENT) then
	SWEP.Category 			= "Team Fortress 2 Custom"
	SWEP.PrintName			= "Crazy Alchemist"
	SWEP.HoldType = "revolver"
	SWEP.Slot				= 0
	SWEP.DrawSecondaryAmmo = false
	SWEP.Type =  "Syringe gun"
	SWEP.Stats =  "On Hit: Poison the target"
	SWEP.Stats2 = "Poison does 5 points of damage."
	SWEP.Stats3 = ""
	SWEP.Stats4 = ""
	SWEP.Stats5 = ""
	SWEP.Stats6 = ""
	...
	SWEP.Stats12 = "+34% fire delay"
	SWEP.Stats13 = ""
	SWEP.Stats14 = ""
	SWEP.Stats15 = ""
	SWEP.Stats16 = ""
	SWEP.Description = "Medic never thought that poison can be so funny to watch."
end
...
SWEP.ViewModelBoneMods = {
	["vm_weapon_bone_1"] = { scale = Vector(0.009, 0.009, 0.009), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
}
SWEP.VElements = {
	["jar+"] = { type = "Model", model = "models/weapons/c_models/c_xms_urinejar.mdl", bone = "vm_weapon_bone_1", rel = "", pos = Vector(0.127, .1, -4.718), angle = Angle(0, 0, 0), size = Vector(0.639, 0.639, 0.639), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.Primary.Sound			= Sound( "weapons/syringegun_shoot.wav" )
SWEP.Primary.CritSound			= Sound( "weapons/syringegun_shoot_crit.wav" )
...
if SERVER then
	...
	local ownerbb = self.Owner
	local trace = self.Owner:GetEyeTrace()
	if (trace.Entity:IsNPC() or trace.Entity:IsPlayer()) and trace.Entity:IsValid() then
	timer.Create("syringeacid"..trace.Entity:EntIndex()..""..math.random(1,15),.5,5,function()
	if not trace.Entity:IsValid() then
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."1")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."2")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."3")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."4")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."5")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."6")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."7")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."8")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."9")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."10")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."11")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."12")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."13")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."14")
	timer.Remove("syringeacid"..trace.Entity:EntIndex().."15")
	end
	trace.Entity:EmitSound("player/resistance_light"..math.random(1,4)..".wav")
	if SERVER then
	trace.Entity:TakeDamage(3,ownerbb,self)
	end
	end)
	end
end
...
if ( self.Reloadaftershoot > CurTime() ) then return end 
	...
	self:Holster2()
	self.Weapon:DefaultReload(self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("sg_reload")))
	self.Idle = CurTime() + self.Owner:GetViewModel():SequenceDuration()
	self.Reloadaftershoot = CurTime() + self.Owner:GetViewModel():SequenceDuration()
	self.Weapon:SetNextPrimaryFire(CurTime() + self.Owner:GetViewModel():SequenceDuration())
...
if (CLIENT) then
self.CModel = ClientsideModel("models/weapons/c_models/c_proto_syringegun/c_proto_syringegun.mdl")
local vm = self.Owner:GetViewModel()
	...
	function SWEP:ViewModelDrawn()
		...
		for k, name in ipairs( self.vRenderOrder ) do
			...
			if (v.type == "Model" and IsValid(model)) then
				...
				model:DrawModel()
				model:SetSubMaterial(0,"models/weapons/c_items/c_urinejar_glass")
				model:SetSubMaterial(1,"models/weapons/c_items/c_breadmonster_goo")
				model:SetSubMaterial(2,"invis")
				render.SetBlend(1)
