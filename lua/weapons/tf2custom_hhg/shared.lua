...
if (CLIENT) then
	...
	SWEP.DrawAmmo		= false
	SWEP.Type =  "Holy grenade"
	SWEP.FirstStat =  "Can be thrown and after hits something, it stops and then explodes."
	SWEP.Stats =  ""
	SWEP.Stats2 = ""
	SWEP.Stats3 = ""
	SWEP.Stats4 = ""
	...
	SWEP.Stats11 = "Has a cooldown of 30 seconds"
	SWEP.Stats12 = ""
	SWEP.Stats13 = ""
	SWEP.Stats14 = ""
	SWEP.Stats15 = ""
	SWEP.Stats16 = ""
	SWEP.Description = "The best fighter of "
	SWEP.Set = "God raiser"
end

SWEP.VElements = {
	["gun"] = { type = "Model", model = "models/player/items/soldier/worms_gear.mdl", bone = "weapon_bone", rel = "", pos = Vector(-39.993, 46.541, -23.268), angle = Angle(-7.382, -29.476, -69.503), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
SWEP.WElements = {
	["gun"] = { type = "Model", model = "models/player/items/soldier/worms_gear.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(11.164, -30.043, 53.37), angle = Angle(180, -1.009, 29.24), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrintWeaponInfo( x, y, alpha )
	...
	--if (self.InfoMarkup == nil ) then
		...
		local god_color = "<color=255,0,127,255>"
		local yellow_color = "<color=255,255,0,255>"
		local green_color = "<color=0,255,0,255>"
		
		str = "<font=HudSelectionText>"
		if ( self.Type != "" ) then str = str ..god_color.."Limited edition level "..self.LVLNumber.." "..self.Type.."</color>\n" end
		--end
...
function SWEP:Throw()
    if SERVER then
        ...
        local grenade = ents.Create("tf2custom_proj_hhg")
		if self.Owner:GetEyeTrace().HitPos:Distance(self.Owner:GetShootPos()) < 80 then
		grenade:SetPos(self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 10)
		else
		grenade:SetPos(self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 80)
		end
	...
	self:Remove()
	timer.Create("hhgrespawn"..oldowner:EntIndex(),15,1,function()
	if SERVER then
	oldowner:Give("tf2custom_hhg")
	end
	end)
end

hook.Add("PostPlayerDeath","TF2Customgrenadexmasgiveply",function(ply)
if timer.Exists("hhgrespawn"..ply:EntIndex()) then timer.Remove("hhgrespawn"..ply:EntIndex()) end
end)
	...
	function SWEP:ViewModelDrawn()
		...
		for k, name in ipairs( self.vRenderOrder ) do
			...
			if (v.type == "Model" and IsValid(model)) then
				...
				model:DrawModel()
				model:ManipulateBoneScale( model:LookupBone("prp_grenade_bottom"), Vector(0,0,0) )
				model:ManipulateBoneScale( model:LookupBone("prp_helmet"), Vector(0,0,0) )
				render.SetBlend(1)
