...
if (CLIENT) then
	...
	SWEP.DrawAmmo		= false
	SWEP.Stats =  "+15% attack speed rate"
	...
	SWEP.Stats15 = ""
	SWEP.Stats16 = ""
	SWEP.Description = "The Samson Skewer is a knife that does more damage, but has a short range."
end

--[[
lua_run local time = os.date("*t",os.time())PrintTable(time) print(2) local time2 = {day = 30,hour = 16,time.isdst,min = 4,month = 7,sec = 11,wday = 7,yday = 212,year = 2016} PrintTable(time2) if time == time2 then print("ok") else print("no") end
]]--


SWEP.VElements = {
	["hat"] = { type = "Model", model = "models/player/items/all_class/xms_santa_hat_heavy.mdl", bone = "vm_weapon_bone_1", rel = "", pos = Vector(3, 0, 0), angle = Angle(-112.448, 14.098, -6.432), size = Vector(0.978, 0.978, 0.978), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrimaryAttack()
	...
	if self.Weapon:GetNWBool("Critical") then
	self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("breadglove_swing_crit"))
	self.Weapon:EmitSound("Weapon_Machete.MissCrit")
	else
	self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("breadglove_swing_left"))
...
function SWEP:SecondaryAttack()
	...
	self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("breadglove_swing_right"))
	self.Weapon:EmitSound("Weapon_Machete.Miss")
	end
	self.Owner:SetAnimation(PLAYER_ATTACK1)
	self.DoMelee = CurTime() + 0.22
...
function SWEP:Deploy()
	...
	self.Weapon:SetNWBool("Critical",false)
	self.DoMelee = nil
	self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("breadglove_draw_b"))
	self.Idle = CurTime() + self.Owner:GetViewModel():SequenceDuration()
...
self.Idle = nil
self.Owner:GetViewModel():SetSequence(self.Owner:GetViewModel():LookupSequence("breadglove_idle_b"))
end
end
...
if (CLIENT) then
self.CModel = ClientsideModel("models/weapons/c_models/c_breadmonster_gloves/c_breadmonster_gloves.mdl")
local vm = self.Owner:GetViewModel()
